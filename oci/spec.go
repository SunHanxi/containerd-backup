...
c: 容器实例。
s: 要修改的规范（Spec）实例。
opts ...SpecOpts: 可变参数，表示要应用的一系列规范选项（SpecOpts）。
函数功能：
函数使用一个循环遍历传递进来的规范选项列表，并对每个选项调用相应的SpecOpts函数。这些函数在前一个注释中定义，它们负责将特定于规范的信息设置到规范对象中。

错误处理：
如果在调用任何规范选项时发生错误，函数将立即返回错误。否则，如果所有选项都成功应用，函数返回nil，表示应用操作成功。

这种模式允许通过ApplyOpts函数集中地应用一系列规范选项，同时提供了扩展性，因为可以随时添加新的规范选项。这种设计模式在容器化和OCI规范中很常见，因为它允许在不修改主要逻辑的情况下轻松地添加或修改容器的配置和行为。
*/
