...
// 这个方法通过给定的ID创建一个新的容器，注意，这个容器ID必须是命名空间内唯一的。
// containerd不会自动为你生成ID，docker会，docker底层也是调用的containerd，带了很多参数进来。
// run方法解析完命令行参数之后，就跳转到了这儿，id是容器id，opts里面塞了一堆oci标准的spec。
// 注意，这里面存的是容器规范，还不是OCI规范呢。
func (c *Client) NewContainer(ctx context.Context, id string, opts ...NewContainerOpts) (Container, error) {
	...
	//初始化container对象，先把id和runtime放进去。
