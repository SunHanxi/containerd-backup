...
func NewContainer(ctx gocontext.Context, client *containerd.Client, context *cli.Context) (containerd.Container, error) {
	...
	//如果config存在，就解析，否则挨个解析命令行参数
	if config {
		//从config里拿到label，然后把容器标签标准塞进去，后面校验标签是否符合标准。
		cOpts = append(cOpts, containerd.WithContainerLabels(commands.LabelArgs(context.StringSlice("label"))))
		// 这份配置文件需要符合oci标准，所以传递进去oci 配置文件标准
		// 看了内部的实现，似乎就是看能不能用json解析出来，能解析就OK。
		...
		//先塞一些默认的oci规范进去。
		// 首先是oci默认规范，然后因为是unix平台，所以塞进去unix设备规范。
		opts = append(opts, oci.WithDefaultSpec(), oci.WithDefaultUnixDevices)

		//从文件中解析环境变量，解析出来之后，用oci env文件规范检查。
		// 这个规范其实就是把文件读出来，转成kv的字符串数组，然后用oci的env规范检查。
		...
		//fmt.Println(context.FlagNames())

		//从命令行中解析环境变量
		// 先注释掉，方便测试，此时命令行的环境变量无法传递到容器内
		//opts = append(opts, oci.WithEnv(context.StringSlice("env")))

		env := []string{"a=a", "b=b", "c=c"}

		// 扫进去oci env规范
		// 因为环境变量和进程有关，所以规范实现的时候，是把env set到了process里。
		// 然后用 replaceOrAppendEnvValues 方法处理
		opts = append(opts, oci.WithEnv(env))

		// 容器要挂载文件系统进去，所以塞进去oci 挂载规范
		opts = append(opts, withMounts(context))

		// 一个对rootfs的特殊处理，没看懂。
		// 似乎是容器可以从rootfs创建，也可以从快照恢复。
		// 如果是快照恢复，就需要处理一堆快照解压校验什么的。
